...
const serve = () => {
  watch(config.build.paths.style, style);
  watch(config.build.paths.script, script);
  watch(config.build.paths.page, page);

  watch(
    [config.build.image, config.build.font],
    { cwd: config.build.src },
    bs.reload
  );

  watch('**', config.build.public, bs.reload);
  bs.init({
...
module.exports = { build, serve, clean };
